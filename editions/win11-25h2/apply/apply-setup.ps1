...
try {
    ...
    $oneDriveInstaller = $oneDrivePaths | Where-Object { Test-Path $_ } | Select-Object -First 1
    if ($oneDriveInstaller) {
        Invoke-External -FilePath $oneDriveInstaller -Arguments '/uninstall' -IgnoreExitCode
    }
    ...
    if ($script:HadFatalError) {
        Write-Host "    - Apply Setup completed with errors." -ForegroundColor Yellow
    } else {
        Write-Host "    - Apply Setup Complete." -ForegroundColor Green
    }
}